...
对一个质点组，其由两个以上质点组成，定义此质点组的质心，记为 $c$.在某个参考系$K$中，定义质心的位矢为$\pmb{r_c}$,其定义如下：
$$\pmb{r_c}=\sum_i\frac{m_i\pmb{r_i}}{m}$$
其中:
$$m=\sum_i m_i$$
$m_i$是质点组中第$i$个质点的质量，$\pmb{r_i}$是质点组中第$i$个质点在某个参考系$K$中的位矢，由上面对$m$的定义知，$m$是质点组的总质量,所有位矢都以同一个点$K$为起点(因为位矢$\pmb{r_i}$和 $\pmb{r_c}$是在参考系$K$中的位矢).

质心位矢的其他表述方式：在某个参考系中，一个质点组的质心的位矢等于质心组中所有质点在此参考系中的位矢以质量为权的平均(矢量).

在直角坐标系中，可将质点组的质心的位矢表达式写为分量形式. 在某个参考系中，记质心的位矢$\pmb{r_c}=(x_c,y_c,z_c)$, 质点组中第$i$个质点的位矢$\pmb{r_i}=(x_i,y_i,z_i)$则有：
$$x_c=\sum_i \frac{m_ix_i}m$$
$$y_c=\sum_i \frac{m_iy_i}m$$
$$z_c=\sum_i \frac{m_iz_i}m$$

在某个参考系中，质心$c$的速度记为$\pmb{v_c}$,不考虑相对论效应(即质点的质量不随质点速度的变化而变化)，则由速度的定义:
$$\pmb{v_c}=\frac{d \pmb{r_c}}{dt}=\frac{d}{dt}\sum_i \frac{m_i\pmb{r_i}}m=\frac{1}{m}\sum_i \frac{d}{dt}m_i\pmb{r_i}=\frac{1}{m}\sum_i m_i\frac{d}{dt}\pmb{r_i}=\frac{1}{m}{\sum_i m_i\pmb{v_i}}$$
质心速度的其他表述：在某个参考系中，一个质心组的质点的速度等于质心组中各质点在此参考系中的速度以质量为权的平均.

记$\pmb{p_i}=m_i\pmb{v_i}$为质心组中第$i$个质点的动量，则对上等式两边同乘$m$得：
$$m\pmb{v_c}=\sum_i m_i\pmb{v_i}$$
定义一个质点组的质心的动量为$\pmb{p_c}=m\pmb{v_c}$,对上等式两边分别替换成动量有：
$$\pmb{p_c}=\sum_i \pmb{p_i}$$
这意味着，在任意一个参考系中，一个质点组的质心的动量等于质点组中各质心的动量之和.

特别地，假设在某个参考系中，某个质点组的第$i$个质点的速度为$\pmb{v_i}$,这个质点组的质心的速度为$\pmb{v_c}$ ; 若以质心为参考系，一个质心组的第$i$个质点在质心参考系中的速度为$v'_i$,则由伽利略变换：
$$\pmb{v'_i}=\pmb{v_i}-\pmb{v_c}$$
上式左右同乘$m_i$
$$m_i\pmb{v'_i}=m_i\pmb{v_i}-m_i\pmb{v_c}$$
求和：
$$\sum_i m_i\pmb{v'_i}=\sum_i m_i\pmb{v_i}-\sum_im_i\pmb{v_c}$$
化简：
$$\sum_i m_i\pmb{v'_i}=\sum_i m_i\pmb{v_i}-\pmb{v_c}\sum_i m_i$$
$$\sum_i m_i\pmb{v'_i}=\sum_i m_i\pmb{v_i}-m\pmb{v_c}$$
$$\sum_i m_i\pmb{v'_i}=\sum_i \pmb{p_i}-\pmb{p_c}$$
$$\sum_i m_i\pmb{v'_i}=\pmb{0}$$
亦即：
$$\sum_i \pmb{p'_i}=\pmb{0}$$
上式意味着，在质心参考系中，各质点的动量和为零.
### 4.2质心运动定理
假设在某个惯性参考系中，对一个质点组中的每个质点用牛顿第二定律并求和，其中$F_i$是第$i$个质点受到的**合外力**
$$\sum_i\pmb{F_i}=\sum_im_i\pmb{a_i}$$
定义在某个参考系中，一个质心组的质心的加速度为：
$$a_c=\frac{d\pmb{v_c}}{dt}=\frac{d}{dt}(\frac{1}{m}\sum_im_i\pmb{v_i})=\frac{1}{m}\sum_i m_i\frac{d\pmb{v_i}}{dt}=\frac{1}{m}\sum_i m_i\pmb{a_i}$$
这意味着，一个质点组的质心的加速度等于这个质点组中各质心加速度以质量为权的平均.
对上式左右两边同乘m有：
$$m\pmb{a_c}=\sum_i m_i\pmb{a_i}$$
结合上上一行，消去相同项，有：
$$\sum_i\pmb{F_i}=ma_c$$
记这个质点组所受的合外力为$F_外$(对质心来说，不存在内力，之所以所有质心所受力之和为质心组这个系统所受合外力，是因为由牛顿第三定律，任意两个质心的相互作用在求和时抵消了)，有：
$$\pmb{F_外}=ma_c$$
